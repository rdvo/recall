...
function normalizeToolPart(
  ...
  if (part.tool === 'write' && input.content) {
    ...
    resultText = String(input.content).slice(0, 100000); // 100KB for full files
...
function normalizeMessage(
  ...
  // If this is a user message with no parts, create a placeholder
  // (user messages may have their text directly in parts)
  
  for (const part of parts) {
    if (part.type === 'tool') {
      const toolEvents = normalizeToolPart(part as OpenCodeToolPart, message, sourceSeq + seqOffset, ctx);
      events.push(...toolEvents);
      seqOffset += toolEvents.length;
    } else if (part.type === 'text') {
      events.push(normalizeTextPart(part as OpenCodeTextPart, message, sourceSeq + seqOffset, ctx));
      seqOffset++;
    } else if (part.type === 'reasoning') {
      events.push(normalizeReasoningPart(part as OpenCodeReasoningPart, message, sourceSeq + seqOffset, ctx));
      seqOffset++;
    }
...
export function ingestOpenCodeSession(
  ...
  if (cursor?.file_mtime && currentMtime <= cursor.file_mtime) {
  ...
  const newMessages = afterTimestamp 
    ? messages.filter(m => m.time.created > afterTimestamp)
    : messages;
  ...
  // Build new cursor - map to standard fields:
  // - file_mtime = session modification time
  // - byte_offset = last message timestamp  
  // - last_event_id = last message ID
  const lastMessage = messages[messages.length - 1];
  const newCursor: OpenCodeCursor = {
    source_id: ctx.sourceId,
    last_event_id: lastMessage?.id,
    byte_offset: lastMessage?.time.created,
